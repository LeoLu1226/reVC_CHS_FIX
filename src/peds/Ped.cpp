...
{
	if (CClassicAxis::Aiming(this)) return true;
#ifdef FREE_CAM
	if (CCamera::bFreeCam)
		return false;
#endif
...
CPed::FinishLaunchCB(CAnimBlendAssociation *animAssoc, void *arg)
{
	...
	if (ped->IsPlayer() || ped->m_pedInObjective && ped->m_pedInObjective->IsPlayer())
		ped->ApplyMoveForce(0.0f, 0.0f, 8.5f);//跳跃高度
	else
	...
	if (sq(velocityFromAnim) > ped->m_vecMoveSpeed.MagnitudeSqr2D() || ped->m_pCurrentPhysSurface) {
		...
		if (TheCamera.Cams[0].Using3rdPersonMouseCam()) {
			...
			ped->m_vecMoveSpeed.x = -velocityFromAnim * Sin(fpsAngle);//跳跃后的速度
			ped->m_vecMoveSpeed.y = velocityFromAnim * Cos(fpsAngle);//跳跃后的速度
		} else {
